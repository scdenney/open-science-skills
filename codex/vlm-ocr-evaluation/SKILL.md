...
- Record an exact registry entry per system — name, HuggingFace id or dated API identifier, quantization, and loader — one record per model, keyed by id rather than family name. Family-name-only reporting ("we used Qwen and Gemma") is not reproducible.
