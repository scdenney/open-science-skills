...
Optionally link `CLAUDE.md` to `AGENTS.md` so Claude Code reads the same canonical instructions:
...
The spine only stays trustworthy if every new source flows through the same pipeline. Write it into `sources/README.md` and the repo-local `process-source` skill, and state it to the user:
...
The per-PDF mechanics belong to `process-source`, which the scaffolded `.agents/skills/process-source/SKILL.md` points at. This skill guarantees the pipeline *exists and is wired*; `process-source` *runs* it.
...
- **`process-source`** — run the per-PDF intake into the structure this skill creates: the repo-local skill scaffolded in Step 3, or a global equivalent. Use every time a new paper arrives.
