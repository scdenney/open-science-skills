...
`Glob` for `.codex/prompts/*finish*.md`, `.codex/prompts/*wrap*.md`, and the same patterns under `.claude/commands/` — they catch variants such as `finished.md` or `wrap-up.md`. If one matches, `Read` it, follow its instructions instead of the steps below, and say which file you used. Otherwise continue here.

## 1. Learn the project's conventions

Read `AGENTS.md` and `CLAUDE.md` for the stated handoff or logging convention, then locate the artifacts the project actually keeps: `HANDOFF.md`, `CODEX_HANDOFF.md`, `STATUS.md`, `SESSION_LOG.md`, `NOTES.md`, or a dated `logs/` directory. Match the structure already in the file. If the project keeps none, summarize in the reply and ask **once** whether to start a durable file — do not create one unprompted.
