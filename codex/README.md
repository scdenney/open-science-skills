...
- `$46-orchestrate` is explicit-invocation only because it fans work out to subagents. It routes by role, risk, and verifiability, and pins each subagent to a GPT-5.6 tier through the bundled `scripts/codex-worker.sh` — `gpt-5.6-luna` (fast/cheap) for fully-specified implementer work, `gpt-5.6-terra` (balanced) for analyst/verifier consults and as the default, and two parallel `terra` calls (swappable for `gpt-5.6-sol` once/if that gate lifts) for the two-independent-check high-stakes path. The lead ("Sol") is the running Codex session itself and is never spawned via the script; `luna` is the intended cheap tier but is untested in this repo, and `sol` remains rejected outright under this account's ChatGPT-based Codex auth.
