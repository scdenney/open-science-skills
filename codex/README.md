...
One sandbox constraint applies to all three variants. Their `codex-member.sh` (GPT-5.6 Sol, or Terra for `model-committee-sol`'s debater) needs an unsandboxed or escalated call under a live Codex session, since a nested `codex exec` under sandbox fails structurally (see each `SKILL.md`). `claude-member.sh` was observed to hang under sandbox for the same reason (network access), less rigorously confirmed.
