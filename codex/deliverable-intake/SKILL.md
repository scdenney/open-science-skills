...
Five lines: captures read; counts by class; UNRESOLVED items (each with what would resolve it); decisions applied; what to run next (`$deliverable-lint` if the wiki changed materially, otherwise nothing).
...
Single worker, no delegation. Present the complete patch before assent and apply only the approved hunks. Never load dictation-log entries the user did not name. Files and formats are identical to the Claude skill.
