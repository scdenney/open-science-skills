...
**Honest caveat, carried from the script's header:** `sol` (flagship) has been rejected outright on some ChatGPT-account Codex auth setups ("not supported when using Codex with a ChatGPT account") — confirm on your own account before relying on it. `luna` and `terra` are both confirmed working (smoke-tested directly: `scripts/codex-worker.sh --tier luna --mode consult --prompt "reply OK" -C .`), **but only on a current Codex CLI** — an outdated CLI rejects both tiers with "requires a newer version of Codex." Run `codex --version` and update if delegation calls fail immediately; a failed delegation call means the lead falls back to doing the work itself, silently burning far more tokens than tiering was meant to save.
