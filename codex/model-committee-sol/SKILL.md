...
Invoked as `$model-committee-sol` inside a Codex/Sol session, Sol chairs natively — that is this variant's intended home. Sol is neither deliberating member: cross-family to the Opus member, and a distinct 5.6 tier from the Terra member. The GPT member is pinned to Terra rather than Sol precisely because the chair is Sol; a chair identical to a member would defeat the point. Siblings: [`model-committee`](../model-committee/SKILL.md) (Opus 5 chairs) and [`model-committee-fable`](../model-committee-fable/SKILL.md) (Fable 5 chairs).
...
These are exact pins, not moving aliases. If one is unavailable, report it and ask whether to stop or use a named replacement — never substitute silently, and never fall back to `gpt-5.6-terra` for the chair, since a chair identical to a member defeats the point of this variant.
...
You are Sol, so chair directly after round 3. Running this skill outside a Sol session, delegate only the post-round-3 chair step:
