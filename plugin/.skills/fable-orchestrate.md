...
| deep-reasoner | `high`, pinned | `effort: high` in `agents/deep-reasoner.md` — a bounded hard-reasoning shot, not a sustained role; pinned explicitly rather than inheriting the session's own effort, since early Opus 5 field reports suggest sustained high-effort roles are where it underperforms, and a bounded fan-out shot doesn't need to match the Fable lead's own `max` |
