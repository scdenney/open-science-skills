...
**Orchestration lead.** This review is orchestrated by whatever model you are running — Claude Opus 4.8 or Fable 5. The orchestrator does the orientation, launches the parallel dimension sub-agents, runs the cross-check cascade, and writes the synthesis; the sub-agents do the reading. **If you are on Opus, run it under ultracode** (xhigh reasoning + dynamic Workflow fan-out): express Phase 2 (nine parallel dimensions) and Phase 3 (cross-check) as a `Workflow` — a `pipeline()` so each dimension's finding verifies as soon as that dimension completes — and reserve your xhigh reasoning for the Phase 4 adjudication, deduplication, and Editor's Note. On a lighter lead (Fable), drive the same phases with parallel `Agent` calls and let Opus sub-agents carry the argument-level dimensions. Either way, pin mechanical dimensions (DOIs, figures, formatting) to Sonnet and argument-level dimensions (content, numbers, CONSORT, pre-reg) to Opus; see [`opus-orchestrate`](../opus-orchestrate/SKILL.md) / [`fable-orchestrate`](../fable-orchestrate/SKILL.md) for the routing.
