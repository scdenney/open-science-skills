...
**Orchestration lead.** This review is orchestrated by whatever model you are running — Claude Opus 5 or Fable 5. The orchestrator does the orientation, launches the parallel dimension sub-agents, runs the cross-check cascade, and writes the synthesis; the sub-agents do the reading. **If you are on Opus, run at medium reasoning effort by default** (raise to high for the Phase 4 adjudication, deduplication, and Editor's Note — a bounded judgment call, not the sustained orchestration role): express Phase 2 (nine parallel dimensions) and Phase 3 (cross-check) as a `Workflow` — a `pipeline()` so each dimension's finding verifies as soon as that dimension completes; invoking this skill already satisfies the `Workflow` tool's own opt-in, so Claude Code's `ultracode` session mode isn't required. On a lighter lead (Fable), drive the same phases with parallel `Agent` calls.

**Sub-agent model and effort assignment.** Route by task type, not by agent number — argument-level dimensions (evaluating logical structure, cross-referencing claims, detecting silent discrepancies) go to Opus at `high`, pinned; mechanical dimensions (matching, existence checks, checklist audits) go to Sonnet at `medium`, pinned. This is a bounded single-shot call per sub-agent, not a sustained role, so pin the effort explicitly on every `Agent` call rather than letting it inherit the orchestrator's own (lower, sustained-role) effort — see [`opus-orchestrate`](../opus-orchestrate/SKILL.md)'s Effort calibration section for the same reasoning. The Phase 3 cross-checker groupings already track this split exactly:
...
| Agent 10 — Content/Numbers/Design Cross-Checker | Opus | `high`, pinned | re-verifies Agents 1, 2, 6, 7's argument-level findings — same tier as its source agents |
...
See [`opus-orchestrate`](../opus-orchestrate/SKILL.md) / [`fable-orchestrate`](../fable-orchestrate/SKILL.md) for the underlying mechanical-vs-argument-level routing pattern this table applies.
