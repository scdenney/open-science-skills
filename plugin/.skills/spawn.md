...
herdr agent explain "$slug"                                      # wait for state: idle — the agent record lags the screen by a few seconds; prompt returns agent_blocked until then
...
7. **Confirm nothing is left**, in three places. `git worktree list` in the main checkout shows only the main checkout and any peers still running. `ls ~/.herdr/worktrees/<repo>/` shows no directory for this slug (on 0.9 the checkout is named after the branch, `spawn-<slug>`; on 0.8 it was `worktree-<adjective>-<noun>-<hex>`); `rmdir` the parent if it is now empty, since `worktree remove` leaves it. `herdr worktree list` and `herdr workspace list` show no workspace for this spawn; `worktree create` also opens a workspace for the source checkout, which nothing else closes, so `herdr workspace close <id>` (positional, not `--workspace`) when the spawn opened one. Since 0.9 that close is refused with `workspace_group_close_required` while any worktree workspace of the repo is still open; `--group` closes the source and every linked worktree workspace at once but leaves their git checkouts on disk, so remove the checkouts first and close the source last.
...
- herdr surface verified 2026-08-06 by live run on herdr 0.7.5; re-verified 2026-09-02 by a live dry run on herdr 0.8.0 (protocol 19, Claude and Codex integration hooks v7), which added `agent explain`, `agent get`, `worktree open`, and `integration`; re-verified 2026-09-08 by a live dry run on herdr 0.9.0 (protocol 22, Claude hook v9, Codex hook v8) in an isolated named session. 0.9 changed four things recorded above: `agent start` exits 1 with `agent_not_ready` on the trust dialog, the agent record lags the screen after the dialog clears, checkouts are named `spawn-<slug>`, and closing a source workspace with open worktree workspaces requires `workspace close --group`. `worktree create|open|remove|list` also gained `--trust-repository` for repositories git refuses as untrusted, and `create|open` take `--focus|--no-focus`. The 0.8 dry run is what established the `--cwd` requirement, the trust-dialog block, and the `explain` output shape above. Flag names above match the 0.9.0 schema (`worktree create`: `--branch --label --base --path --cwd --focus --no-focus --trust-repository`; `worktree remove`: `--workspace --force --trust-repository`; `agent start`: `--kind --pane --timeout`; `agent wait`: `--until --timeout`; `workspace close`: `<id> --group`). The fold-back checklist and orphan sweep were written after a sweep found ten orphaned checkouts across three repos.
