...
argument-hint: '[optional: a subsystem or question to focus the report on]'
---
...
`Glob` for `.claude/commands/*sitrep*.md` and `.claude/commands/*status*.md` — the patterns catch variants such as `project-sitrep.md`. If one matches, `Read` it, follow its instructions instead of the steps below, and say which file you used. If several match, pick the closest name and say which. Otherwise continue here.

## 1. Learn the project's conventions

Read `CLAUDE.md` and `AGENTS.md` if present; they usually state the handoff or logging convention. Fall back to `README.md` for orientation. Then find the project's own memory: `HANDOFF.md`, `CODEX_HANDOFF.md`, `STATUS.md`, `SESSION_LOG.md`, `NOTES.md`, or a `logs/` directory with dated entries.
...
If `CLAUDE.md` or `README.md` names specific data files, build outputs, or directories as load-bearing, spot-check that they exist. Skip this entirely when the project states no such convention.
