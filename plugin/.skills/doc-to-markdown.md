...
Conversion is a cost, not a default. Read the file directly with the Read tool when the question is bounded — Read handles PDFs natively, sees figures, equations, and layout that no text conversion preserves, and takes a `pages` range.
