...
description: Editorial review of a whole deliverable (a talk, a course module, a paper, a chapter, a referee report) against its own manifest, run as detection only - never a rewrite. Runs the deterministic gate first (build, citations, numbers, facts, leaks) and stops on a failure; then fans one cheap finder out per section (clarity and voice tells, claim-to-source support, numeric and terminological consistency); then one strong-model pass over the assembled whole (argument order and flow, spec conformance, repetition and contradiction, release rules); then adjudicates into checks/<date>/findings.json and report.md with anchors, severities, and honest NOT-CHECKED coverage under a dollar budget. Use when the user says "lint the deliverable", "review the whole talk/module/paper", "night shift", "pre-ship review", or before a release. Revision stays serial and in the author's hands.
