...
Run a cross-model adversarial pre-submission audit. Claude and Codex (GPT-5.6 "Sol" at xhigh effort) independently apply the `paper-review-lite` specification (9 review dimensions, Critical-Reviewer posture, quote-grounded findings), then each model cross-checks the other's findings. The final report annotates every retained Critical or Recommended issue with confidence (Mutual, Asymmetric-confirmed, or Asymmetric-after-adjudication) and drops hallucinations on either side.
