...
Pass `--codex` to run the same specification across two model families: Claude and Codex (GPT-6 Astra at xhigh effort) review independently, then each cross-checks the other, and every surviving Critical or Recommended issue is annotated by confidence. Roughly 2× the calls.
