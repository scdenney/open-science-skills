...
- **Why an Opus lead at all, versus Fable?** Both skills now reason in place — the lead makes the hard call itself and delegates only execution and parallel work. The difference is the lead model: `fable-orchestrate` leads with Fable, the stronger reasoner, so it is the higher-quality (and no longer the cheaper) orchestrator. Use `fable-orchestrate` when you want the best lead; use this one when you specifically want an Opus lead — its 1M-context window, its ultracode Workflow fan-out, or simply because Opus is the model you have.
