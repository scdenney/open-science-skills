...
├── CLAUDE.md               # project conventions for coding agents
├── AGENTS.md -> CLAUDE.md  # symlink so Codex/others read the same file
├── .gitignore
...
**Repo conventions**
- [ ] `CLAUDE.md` documents the project and its conventions; `AGENTS.md` is a symlink to it (or a copy).
...
4. The next three actions (typically: set up `.venv` and run `convert-sources.sh`; drop the first PDFs in `unprocessed/` and run `process-source`; fill in `CLAUDE.md` placeholders).
...
```markdown
---
description: Process new papers from sources/unprocessed/ through the full intake pipeline
---
