...
[![version](https://img.shields.io/badge/version-2.15.1-blue)](https://github.com/scdenney/open-science-skills/releases)
...
<td>Sol-led Codex orchestration for complex work. Routes bounded research, implementation, and verification to role-based subagents spawned with Codex's native, in-process <code>spawn_agent</code> tool (feature <code>multi_agent</code>, stable) — confirmed working end-to-end, including a real two-implementer-plus-verifier run. Every subagent runs at the lead's own model and reasoning effort (the tool exposes no per-agent model/effort override, confirmed empirically), so the saving is context isolation and parallelism within a real 4-slot concurrency limit, not cost tiering. Uses two parallel subagents, blind to each other, for high-blast-radius work that is difficult to verify; keeps planning, integration, and final accountability with the lead, which is the running Codex session itself and is never spawned as a subagent.</td>
</tr>
<tr>
