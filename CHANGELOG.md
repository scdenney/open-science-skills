...
- **The duplicate slash commands are gone.** Claude Code registers a plugin skill as `/oss:<name>` by itself, so the per-skill wrapper in `plugin/commands/` had been registering a second, identical menu entry for every skill, one showing the SKILL.md description and one showing only the H1. The 41 colliding wrappers are removed. `plugin/commands/` now holds only the ten alias commands. `check.sh` asserts the inverse of what it used to: no command may share a skill's name, and every command must be a declared alias.
